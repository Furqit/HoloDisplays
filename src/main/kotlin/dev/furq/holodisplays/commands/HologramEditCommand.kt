...
object HologramEditCommand : EditCommand() {
    ...
    override fun resetScale(name: String, source: ServerCommandSource) = HologramManager.resetScale(name, source)
    ...
    override fun resetBillboard(name: String, source: ServerCommandSource) = HologramManager.resetBillboard(name, source)
    ...
    override fun resetRotation(name: String, source: ServerCommandSource) = HologramManager.resetRotation(name, source)
    ...
    private fun executeResetUpdateRate(context: CommandContext<ServerCommandSource>): Int {
        val name = StringArgumentType.getString(context, "name")
        HologramManager.resetUpdateRate(name, context.source)
        return 1
    }
    ...
    private fun executeResetViewRange(context: CommandContext<ServerCommandSource>): Int {
        val name = StringArgumentType.getString(context, "name")
        HologramManager.resetViewRange(name, context.source)
        return 1
    }
}