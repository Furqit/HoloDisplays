...
object HologramEditCommand : EditCommand() {
    ...
    override fun updateScale(name: String, scale: Vector3f, source: ServerCommandSource) =
        HologramManager.updateScale(name, scale, source)

    override fun updateBillboard(name: String, mode: String, source: ServerCommandSource) =
        HologramManager.updateBillboard(name, mode, source)

    override fun updateRotation(name: String, pitch: Float, yaw: Float, roll: Float, source: ServerCommandSource) =
        HologramManager.updateRotation(name, pitch, yaw, roll, source)

    override fun updateCondition(name: String, condition: String?, source: ServerCommandSource) =
        HologramManager.updateCondition(name, condition, source)

    override fun openEditGui(player: net.minecraft.server.network.ServerPlayerEntity, name: String) =
